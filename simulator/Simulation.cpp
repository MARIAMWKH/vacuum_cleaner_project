...
void Simulation::runSingleSimulation(const House& house, std::unique_ptr<AbstractAlgorithm> algo, 
    ...
    if (elapsed > std::chrono::milliseconds(maxSteps)) {
        ...
        result.dirtLeft = simHouse.getTotalDirt();
        result.inDock = false;
    }
    result.score = calculateScore(result.steps,result.dirtLeft ,result.finished, result.inDock, maxSteps );
    {
...
int Simulation::calculateScore(int steps, int dirtLeft, bool finished, bool inDock, int maxSteps) const {
    if (steps > maxSteps && !finished) {
        // Case when the task is DEAD
        return maxSteps + dirtLeft * 300 + 2000;
    } else if (finished && !inDock) {
        // Case when the task is FINISHED but NOT in the docking station
